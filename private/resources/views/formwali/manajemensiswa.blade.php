...
<div class="row">
        <div class="col-xs-12">
          <div class="box box-success">
            ...
            <h3 class="box-title"><b>Daftar Siswa | </b>Kelas : {{Session::get('charkelas')}}</h3>
              ...
              <table id="example1" class="table table-bordered table-hover">
                ...
                <tr>
                 <td align="center"><input type="checkbox"  class="flat-red" id="pilihsiswa" name="pilihsiswa[]" value="{{$data->id}}"></td>
                  ...
                  <td width="15%"><center><a href="{{URL('/wali/manajemensiswa/datasiswa/hapus')}}/{{$data->id}}"><button class="btn btn-danger btn-flat" type="button">Hapus</button></a> <a href="{{URL('/wali/manajemensiswa/datasiswa/edit')}}/{{$data->id}}"><button class="btn btn-warning btn-flat" type="button">Edit</button></a></center></td>                
                </tr>
                @endforeach
                </tbody>
                <tfoot>
              ...
              <button type="button" class="btn btn-success btn-flat dropdown-toggle" data-toggle="dropdown" style="margin-top: 10px;">
               ...
               <button type="button" id="bulkoption" disabled="disabled" class="btn btn-warning btn-flat dropdown-toggle pull-right" data-toggle="dropdown" style="margin-top: 10px;">Bulk Option 
                  ...
                  <ul class="dropdown-menu pull-right" role="menu" style="margin-top: -10px">
                    ...
                    <li data-toggle="modal" data-target="#migrating" id="openmigration" onclick="migrating();"><a href="#">Migrasi</a></li>
                  </ul>
             <p id="try"></p>
            </div>
            @endif           
      ...
      <div class="row">
        ...
        <div class="col-md-12">
          ...
          @endif
            ...
            @if(Session::has('status'))
              @else
            {{Form::open(array('role'=>'form','url'=>'wali/manajemensiswa/datasiswa/prosestambahsiswa','enctype'=>'multipart/form-data'))}}
            {{Form::hidden('pilihkelas',Session::get('kelas'),['class'=>'form-control'])}}
              <div class="box-body">
        ...
        <div class="col-md-12">
          ...
          <div class="box box-warning">
            ...
            <!-- form start -->
              ...
              {{Form::open(array('role'=>'form','url'=>'wali/manajemensiswa/datasiswa/proseseditsiswa','enctype'=>'multipart/form-data'))}}
...
<!--============================MODAL FOR MIGRATING STUDENT IN BULK MODE=================================-->
<div class="example-modal">
        <div class="modal fade modal modal-warning" id="migrating" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <button type="button" class="close" id="notificationclose" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">Migrasi Kelas</h4>
              </div>
              <div class="modal-body">
                <div style="text-align: center"><p><h3>Silahkan pilih kelas di mana siswa akan dipindah</h3></p></div>
                {{Form::open()}}
               <div class="form-group">                   
                ...
                {{Form::close()}}
                <div class="modal-footer">
                <button type="button" class="btn btn-success btn-flat" disabled="disabled" id="kamang" data-dismiss="modal" onclick="migratingexe();">Eksekusi</button>  
...
<div class="example-modal">
        <div class="modal fade modal modal-success" id="notification" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title"><p id="pesan"></p></h4>
              </div>
              <div class="modal-body">
                <div style="text-align: center"><p><h3 id="isipesan"></h3></p></div>
...
<div class="example-modal">
        <div class="modal fade modal modal-default" id="import" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
              ...
              <h3 style="display: none; text-align: center;" id="loading">Sedang memuat. Silahkan tunggu</h3><h3 id="wait" style="text-align: center;"></h3>
              {{Form::open(array('role'=>'form','url'=>'wali/manajemensiswa/datasiswa/import','enctype'=>'multipart/form-data'))}}

              {{Form::hidden('id_kelas',Session::get('kelas'))}}
...
<!--========================================MODAL FOR NOTIFICATION=======================================-->
<button type="button" id="shownotification" data-toggle="modal" data-target="#notification" style="display: none"></button>
<button type="button" id="showfailednotification" data-toggle="modal" data-target="#failednotification" style="display: none"></button>
...
<script type="text/javascript">
  ...
  headers: {
    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      ...
      var variabel = "{{Session::get('golongankelas')}}"+"I";

      var dataString = "golkelas="+variabel;
      var alamat = "{{URL('wali/manajemensiswa/bulk/findclass')}}";
        ...
        document.getElementById('kamang').disabled = false;
...
<script type="text/javascript">
  ...
  headers: {
    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      ...
      var a = $("#pilihsiswa:checked").map(function(){return $(this).val();}).toArray();
      var id_kelas = JSON.stringify(a);
      var kelaslama = "{{Session::get('kelas')}}";
      var kelasbaru = document.getElementById('kelas').value;

      var dataString = "id_siswa="+id_kelas+"&kelaslama="+kelaslama+"&kelasbaru="+kelasbaru;
      var alamat = "{{URL('wali/manajemensiswa/bulk/migrateupclass')}}";
        ...
        $.ajax({
            ...
            data: dataString,
            success: function(data){
              var isipesan, pesan, status;
              
              if (data.status == 1) { 
                document.getElementById('isipesan').innerHTML = data.isipesan;
                ...
                document.getElementById('isipesan2').innerHTML = data.isipesan;
              document.getElementById('pesan2').innerHTML = data.pesan;
                document.getElementById('notificationclose').click();           
                document.getElementById('showfailednotification').click();
              }
            }
...
<script type="text/javascript">
  ...
  headers: {
    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      ...
      var id_kelas = "{{Session::get('kelas')}}";

      var dataString = "id_kelas="+id_kelas;
      var alamat = "{{URL('wali/manajemensiswa/bulk/resetmigrateupclass')}}";
        ...
        $.ajax({
            ...
            data: dataString,
            success: function(data){
              document.getElementById('closefailed').click();
              document.getElementById('openmigration').click();
            }

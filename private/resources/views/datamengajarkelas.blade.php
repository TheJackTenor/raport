          ...
          @endif
            ...
            @endif
              <table id="example1" class="table table-bordered table-hover">
                ...
                @foreach($datamengajarkelasjoin as $data3)
                  ...
                  @if($counter=="1")
                    ...
                    @elseif($data3->namaguru !== $memory)
                      <td width="15%"><center><a href="{{URL('/manajemenguru/guru/datamengajarkelas/hapus')}}/{{$memoryid}}"><button class="btn btn-danger btn-flat btn-xs">Hapus</button></a> <a href="{{URL('/manajemenguru/datamengajarkelas/edit')}}/{{$memoryid}}"><button class="btn btn-warning btn-flat btn-xs">Edit</button></a></center></td>
                    @endif
                  @endif               
                  @if($data3->namaguru !== $memory)
                <tr>
                ...
                @endforeach
                @if($memoryid!=="")
                <td width="15%"><center><a href="{{URL('/manajemenguru/datamengajarkelas/hapus')}}/{{$memoryid}}"><button class="btn btn-danger btn-flat btn-xs">Hapus</button></a> <a href="{{URL('/manajemenguru/datamengajarkelas/edit')}}/{{$memoryid}}"><button class="btn btn-warning btn-flat btn-xs">Edit</button></a></center></td>
                @endif
                </tbody>
                <tfoot>
                <tr>
...
<div class="row">
        ...
        <div class="col-md-12">
          ...
          @endif
            ...
            @if(Session::has('status'))
              @else
            {{Form::open(array('role'=>'form','url'=>'/manajemenguru/datamengajarkelas/prosestambahdatamengajarkelas'))}}
              <div class="box-body">
                ...
                <select id="pilihguru" name="pilihguru" class="form-control select2" aria-hidden="true"  style="width: 100%;">
                 @foreach($datamengajarkelasjoin as $data)
                  <option @if($data -> statusmengajarkelas == '1') disabled="disabled" @endif  value="{{$data -> id}}" true>{{$data -> nip}} - {{$data -> namaguru}}</option>
                  @endforeach
                </select>
...
@if(Session::has('status'))
        ...
        <div class="col-md-12">
          ...
          <div class="box box-warning">
            ...
            {{Form::open(array('role'=>'form','url'=>'/manajemenguru/datamengajarkelas/proseseditdatamengajarkelas'))}}
           ...
           <button type="submit" class="btn btn-success btn-flat">Simpan</button>
                 {{Form::close()}}
                 <a href="{{URL::asset('/manajemenguru/datamengajarkelas')}}"><button type="button" class="btn btn-warning btn-flat">Kembali</button></a>      
          </div>
          </div>
          </div>
@endif

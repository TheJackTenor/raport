...
@php
  ...
  @endphp
  {{Form::open(array('role'=>'form','url'=>'identitaskd/proseskdpengetahuan','enctype'=>'multipart/form-data','id'=>'super'))}}
            <div class="box-body">
               <div class="form-group">                  
                    {{Form::label('semester','Semester',['class'=>'control-label'])}}                   
                     <select name="semester" id="semester" class="form-control">
                      @foreach($semesters as $semester)
                        <option value="{{$semester}}" @if(Request::is('identitaskd/pengetahuan')) @if($semester == Session()->get('semester')) selected='selected' @endif @elseif(Request::is('identitaskd/pengetahuan/filter')) @if($semester == Session()->get('semestertemp')) selected='selected' @endif @endif>{{$semester}}</option>
                      @endforeach
                  </select>
                ...
                <div class="form-group">                  
                    ...
                    @foreach($ajaran as $ajarans)
                      <option value="{{$ajarans}}" @if(Request::is('identitaskd/pengetahuan')) @if($ajarans == Session()->get('tahunajaran')) selected='selected' @endif @elseif(Request::is('identitaskd/pengetahuan/filter')) @if($ajarans == Session()->get('tahunajarantemp')) selected='selected' @endif @endif>{{$ajarans}}</option>
                    @endforeach
      ...
      @if(Request::is('identitaskd/pengetahuan/filter'))
      <a href="{{URL('identitaskd/pengetahuan')}}"><button class="btn btn-warning btn-flat pull-right" type="button">Kembali</button></a>
      @endif
            </div>
...
<script type="text/javascript">
  ...
  headers: {
    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      ...
      var alamat = "{{URL('kdpengetahuantrigger')}}";
        ...
        $.ajax({
         ...
         $('#loading').show();
            ...
            $(document).ready(function(){window.setTimeout(function() {
              window.location.href = '{{URL('/identitaskd/pengetahuan/filter')}}';}, 500);
            });

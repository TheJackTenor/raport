...
@php
  ...
  @endphp
  {{Form::open(array('role'=>'form','url'=>'identitaskd/proseskdsosial','enctype'=>'multipart/form-data','id'=>'super'))}}
            <div class="box-body">
               <div class="form-group">                  
                    {{Form::label('semester','Semester',['class'=>'control-label'])}}                   
                     <select name="semester" id="semester" class="form-control">
                      @foreach($semesters as $semester)
                        <option value="{{$semester}}" @if(Request::is('identitaskd/sosial')) @if($semester == Session()->get('semester')) selected='selected' @endif @elseif(Request::is('identitaskd/sosial/filter')) @if($semester == Session()->get('semestertemp')) selected='selected' @endif @endif>{{$semester}}</option>
                      @endforeach
                  </select>
                ...
                <div class="form-group">                  
                    ...
                    @foreach($ajaran as $ajarans)
                      <option value="{{$ajarans}}" @if(Request::is('identitaskd/sosial')) @if($ajarans == Session()->get('tahunajaran')) selected='selected' @endif @elseif(Request::is('identitaskd/sosial/filter')) @if($ajarans == Session()->get('tahunajarantemp')) selected='selected' @endif @endif>{{$ajarans}}</option>
                    @endforeach
      ...
      <img src="{{URL::asset('/private/storage/app/loading.gif')}}" id="loading" style="max-width:30px;max-height:30px;" /> 
      @if(Request::is('identitaskd/sosial/filter'))
      <a href="{{URL('identitaskd/sosial')}}"><button class="btn btn-warning btn-flat pull-right" type="button">Kembali</button></a>
      @endif
            </div>
        ...
        <div class="col-md-9">
          <div class="box box-success">
            <div class="box-header with-border">
              ...
              <button class="btn btn-success" type="button" id="difol"></button>  <b style="color: green">KD Sosial Default</b> || <button class="btn btn-warning" type="button" id="clear"></button>  <b style="color: orange">Kosongkan</b>
              </div>
            </div>
...
<script type="text/javascript">
  ...
  headers: {
    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      ...
      var alamat = "{{URL('kdsosialtrigger')}}";
        ...
        $.ajax({
         ...
         $('#loading').show();
            ...
            $(document).ready(function(){window.setTimeout(function() {
              window.location.href = '{{URL('/identitaskd/sosial/filter')}}';}, 500);
            });

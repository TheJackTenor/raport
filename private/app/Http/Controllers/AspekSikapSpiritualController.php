...
class AspekSikapSpiritualController extends Controller
{
   public function dataaspeksikapspiritual(){
    	...
    	$spiritual = DB::table('nilai')->select('id_siswa','nilai','predikat','id')->where('id_kelas','=',Session()->get('kelas'))->where('id_pelajaran','=',Session()->get('pelajaran'))->where('id_guru','=',Session::get('id_guru'))->where('semester','=',session()->get('semester'))->where('tahunajaran','=',session()->get('tahunajaran'))->where('kd_aspek','=',3)->get();

        $detail = DB::table('nilai')->join('detailnilai', function($join){
            $join->on('nilai.id','=','detailnilai.id_nilai')->where('nilai.id_kelas','=',Session()->get('kelas'))->where('nilai.id_pelajaran','=',Session()->get('pelajaran'))->where('nilai.id_guru','=',Session::get('id_guru'))->where('nilai.semester','=',session()->get('semester'))->where('nilai.tahunajaran','=',session()->get('tahunajaran'))->where('nilai.kd_aspek','=',3);
        })->select('detailnilai.kd','detailnilai.id_nilai')->get();

        $detailComparison = DB::table('nilai')->join('detailnilai', function($join){
            $join->on('nilai.id','=','detailnilai.id_nilai')->where('nilai.id_kelas','=',Session()->get('kelas'))->where('nilai.id_pelajaran','=',Session()->get('pelajaran'))->whereNotNull('nilai.id_karyawan')->where('nilai.semester','=',session()->get('semester'))->where('nilai.tahunajaran','=',session()->get('tahunajaran'))->where('nilai.kd_aspek','=',3);
        })->select('detailnilai.kd','detailnilai.id_nilai')->get();

    	$data_kd = DB::table('kd')->select('kd')->where('id_kelas','=',Session()->get('kelas'))->where('id_pelajaran','=',Session()->get('pelajaran'))->where('semester','=',session()->get('semester'))->where('tahunajaran','=',session()->get('tahunajaran'))->where('kd_kd','=',3)->get();
    	...
    	return View::make('formguru/aspeksikapspiritual')->with('datasiswa',$datasiswa)->with('detail',$detail)->with('spiritual',$spiritual)->with('kontenkd',$data_kd)->with('infonilai',$data_kkm)->with('detailComparison',$detailComparison);
    ...
    public function simpanaspeksikapspiritual(){
        DB::table('nilai')->where('id_guru','=',Session::get('id_guru'))->where('id_kelas','=',session()->get('kelas'))->where('id_pelajaran','=',session()->get('pelajaran'))->where('semester','=',session()->get('semester'))->where('tahunajaran','=',session()->get('tahunajaran'))->where('kd_aspek','=',3)->delete();
        ...
        for ($i=1; $i <= Input::get('saverow') ; $i++) {               
            ...
            DB::table('nilai')->insert($data);
             ...
             $idnilai = DB::table('nilai')->select('id')->where('id_guru','=',Session::get('id_guru'))->where('id_kelas','=',session()->get('kelas'))->where('id_pelajaran','=',session()->get('pelajaran'))->where('id_siswa','=',Input::get('idsiswa'.$i))->where('semester','=',session()->get('semester'))->where('tahunajaran','=',session()->get('tahunajaran'))->where('kd_aspek','=',3)->first();
    ...
    public function upload(Request $request, UploadExcelValidator $validator){
        ...
        Excel::filter('chunk')->selectSheets('Sikap Spiritual','Identitas Sis')->load(storage_path('app/uploadexcel/'.$filename))->chunk(250 , function($reader) {
                ...
                if ($man->nis != "") {
                    ...
                    if ($toggle == 0){
                        ...
                        if ($toggle2 > 2) {
                            ...
                            $idnilai = DB::table('nilai')->select('id')->where('id_guru','=',Session::get('id_guru'))->where('id_kelas','=',session()->get('kelas'))->where('id_pelajaran','=',session()->get('pelajaran'))->where('id_siswa','=',$id_siswa->id)->where('semester','=',session()->get('semester'))->where('tahunajaran','=',session()->get('tahunajaran'))->where('kd_aspek','=',3)->first();

...
class AspekSikapSosialController extends Controller
{
    ...
    public function simpanaspeksikapsosial(){
        ...
        for ($i=1; $i <= Input::get('saverow') ; $i++) {               
            ...
            $checkNilaiBk = DB::table('nilai')->where('id_kelas','=',session()->get('kelas'))->where('id_pelajaran','=',session()->get('pelajaran'))->where('id_siswa','=',Input::get('idsiswa'.$i))->where('semester','=',session()->get('semester'))->where('tahunajaran','=',session()->get('tahunajaran'))->where('kd_aspek','=',4)->whereNotNull('id_karyawan')->count();

            if ($checkNilaiBk != 0) {
                $getBkIdNilai = DB::table('nilai')->select('id')->where('id_kelas','=',session()->get('kelas'))->where('id_pelajaran','=',session()->get('pelajaran'))->where('id_siswa','=',Input::get('idsiswa'.$i))->where('semester','=',session()->get('semester'))->where('tahunajaran','=',session()->get('tahunajaran'))->where('kd_aspek','=',4)->whereNotNull('id_karyawan')->first();
